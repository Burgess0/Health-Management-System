...
/**
 *  健康 模块更新页码、执行更新数据库 */
public partial class Jiankang_updt : AppContent
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        mmm = Db.name("jiankang").find(id); // 根据id从模块中获取的一行数据
    }
    ...
    protected void handler()
    {
        var post = getRequestForm();
                ...
                Db.name("jiankang").update(post);
        var charuid = post["id"];
        Dao.execute("UPDATE jiankang SET bmi=((tizhong/((shengao/100)*(shengao/100)))) WHERE id='"+ Request.Params["id"] +"'");
