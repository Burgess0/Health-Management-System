...
/**
 *  留言板 模块更新页码、执行更新数据库 */
public partial class Liuyanban_updt : AppContent
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if(Request["f"] != null){  // 判断是否有
            handler();  // 处理留言板模块的数据插入
            return;
        }
        var id = Req.get("id"); // 获取id
        mmm = Db.name("liuyanban").find(id); // 根据id从模块中获取的一行数据
    }
    ...
    protected void handler()
    {
        var post = getRequestForm();
                ...
                Db.name("liuyanban").update(post);
        var charuid = post["id"];
        Dao.execute("update liuyanban set zhuangtai='已回复' where id='"+ charuid +"'");
