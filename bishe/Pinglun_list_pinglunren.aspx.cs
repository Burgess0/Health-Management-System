 ...
 *
 */
public partial class Pinglun_list_pinglunren : AppContent
{
    // sql 查询条件
    ...
    public object getList()
    {
        ...
        var     query = Db.name("pinglun").@where(where).order(orderby,sort);
        lists    = query.page(12);
    ...
    // GET pinglun_list.aspx?a=delete
    // 删除数据
    public void delete()
    {
        string id = Request["id"];
        string sql = "DELETE FROM pinglun WHERE id='"+id+"'";
        var dmap = Db.name("pinglun").find(id);
                Dao.execute(sql);
                showSuccess("删除成功");
    }
