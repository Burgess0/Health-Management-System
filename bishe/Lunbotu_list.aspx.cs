 ...
 *
 */
public partial class Lunbotu_list : AppContent
{
    // sql 查询条件
    ...
    public string getWhere()
    {
        string where = " 1=1 ";
                ...
                if(!Req.get("title").Equals("")) {
            where += " AND title LIKE '%"+Req.get("title")+"%' ";
        }
        return where;
    }
    ...
    public object getList()
    {
        ...
        var     query = Db.name("lunbotu").@where(where).order(orderby,sort);
        lists    = query.page(12);
    ...
    // GET lunbotu_list.aspx?a=delete
    // 删除数据
    public void delete()
    {
        string id = Request["id"];
        string sql = "DELETE FROM lunbotu WHERE id='"+id+"'";
        var dmap = Db.name("lunbotu").find(id);
                Dao.execute(sql);
                showSuccess("删除成功");
    }
