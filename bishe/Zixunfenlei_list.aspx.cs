 ...
 *
 */
public partial class Zixunfenlei_list : AppContent
{
    // sql 查询条件
    ...
    public object getList()
    {
        ...
        var     query = Db.name("zixunfenlei").@where(where).order(orderby,sort);
        lists    = query.page(12);
    ...
    // GET zixunfenlei_list.aspx?a=delete
    // 删除数据
    public void delete()
    {
        string id = Request["id"];
        string sql = "DELETE FROM zixunfenlei WHERE id='"+id+"'";
        var dmap = Db.name("zixunfenlei").find(id);
                Dao.execute(sql);
                showSuccess("删除成功");
    }
