 ...
 *
 */
public partial class Dingdan_list_xiadanren : AppContent
{
    // sql 查询条件
    ...
    public object getList()
    {
        ...
        sort    =  Req.get("sort" , "desc");  // 获取升序还是降序
                            ...
                            where += " AND xiadanren='"+Session["username"]+"' ";
        
        
        var     query = Db.name("dingdan").@where(where).order(orderby,sort);
        lists    = query.page(12);
    ...
    // GET dingdan_list.aspx?a=delete
    // 删除数据
    public void delete()
    {
        string id = Request["id"];
        string sql = "DELETE FROM dingdan WHERE id='"+id+"'";
        var dmap = Db.name("dingdan").find(id);
                Dao.execute(sql);
        Dao.execute("DELETE FROM dingdanshangpin WHERE dingdanid='"+ Request["id"] +"'");
