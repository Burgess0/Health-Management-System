 ...
 *
 */
public partial class Fenlei_list : AppContent
{
    // sql 查询条件
    ...
    public string getWhere()
    {
        string where = " 1=1 ";
                ...
                if(!Req.get("fenleiming").Equals("")) {
            where += " AND fenleiming LIKE '%"+Req.get("fenleiming")+"%' ";
        }
        return where;
    }
    ...
    public object getList()
    {
        ...
        var     query = Db.name("fenlei").@where(where).order(orderby,sort);
        lists    = query.page(12);
    ...
    // GET fenlei_list.aspx?a=delete
    // 删除数据
    public void delete()
    {
        string id = Request["id"];
        string sql = "DELETE FROM fenlei WHERE id='"+id+"'";
        var dmap = Db.name("fenlei").find(id);
                Dao.execute(sql);
                showSuccess("删除成功");
    }
