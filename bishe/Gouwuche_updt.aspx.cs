...
/**
 *  购物车 模块更新页码、执行更新数据库 */
public partial class Gouwuche_updt : AppContent
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        mmm = Db.name("gouwuche").find(id); // 根据id从模块中获取的一行数据
    }
    ...
    protected void handler()
    {
        var post = getRequestForm();
                ...
                Db.name("gouwuche").update(post);
        var charuid = post["id"];
        Dao.execute("UPDATE gouwuche SET xiaoji=(goumaishu*jiage) WHERE id='"+ Request.Params["id"] +"'");
