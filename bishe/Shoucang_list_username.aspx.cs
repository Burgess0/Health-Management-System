 ...
 *
 */
public partial class Shoucang_list_username : AppContent
{
    // sql 查询条件
    ...
    public object getList()
    {
        ...
        sort    =  Req.get("sort" , "desc");  // 获取升序还是降序
                            ...
                            where += " AND username='"+Session["username"]+"' ";
        
        
        var     query = Db.name("shoucang").@where(where).order(orderby,sort);
        lists    = query.page(12);
    ...
    // GET shoucang_list.aspx?a=delete
    // 删除数据
    public void delete()
    {
        string id = Request["id"];
        string sql = "DELETE FROM shoucang WHERE id='"+id+"'";
        var dmap = Db.name("shoucang").find(id);
                Dao.execute(sql);
                showSuccess("删除成功");
    }
