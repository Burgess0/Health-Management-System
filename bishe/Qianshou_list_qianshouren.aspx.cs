 ...
 *
 */
public partial class Qianshou_list_qianshouren : AppContent
{
    // sql 查询条件
    ...
    public object getList()
    {
        ...
        sort    =  Req.get("sort" , "desc");  // 获取升序还是降序
                            ...
                            where += " AND qianshouren='"+Session["username"]+"' ";
        
        
        var     query = Db.name("qianshou").@where(where).order(orderby,sort);
        lists    = query.page(12);
    ...
    // GET qianshou_list.aspx?a=delete
    // 删除数据
    public void delete()
    {
        string id = Request["id"];
        string sql = "DELETE FROM qianshou WHERE id='"+id+"'";
        var dmap = Db.name("qianshou").find(id);
                Dao.execute(sql);
                showSuccess("删除成功");
    }
