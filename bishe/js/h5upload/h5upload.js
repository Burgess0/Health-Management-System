...
function openAttachmentLayer(url, param, callback) {
    ...
    layer.open({
        ...
        shadeClose: true,
        shade: 0.5,
        content: url + "?" + $.param(param) + "&callback=" + key,
    });
}

function showUploadImages(id, url) {
    $(id).hide();
    url = url || "upload.html";
    var $tpl = $(
    ...
    $tpl.on("click", ".uploadBtn,.thumb,.btnClose", function (e) {
        if ($(this).hasClass("uploadBtn")) {
            ...
            openAttachmentLayer(url, param, function (values) {
                appendImages($gellay, values);
                updateInputValue();
            });
    ...
    function appendImages($gellay, obj) {
        ...
        var $tpl = $(
            '<div class="uploadImage" draggable="true">' +
                '<span class="thumb thumbnail"><img src="' +
                obj.filepath +
                '"/></span>' +
                '<span class="btnClose">' +
