 ...
 *
 */
public partial class Leixing_list : AppContent
{
    // sql 查询条件
    ...
    public string getWhere()
    {
        string where = " 1=1 ";
                ...
                if(!Req.get("leixingming").Equals("")) {
            where += " AND leixingming LIKE '%"+Req.get("leixingming")+"%' ";
        }
        return where;
    }
    ...
    public object getList()
    {
        ...
        var     query = Db.name("leixing").@where(where).order(orderby,sort);
        lists    = query.page(12);
    ...
    // GET leixing_list.aspx?a=delete
    // 删除数据
    public void delete()
    {
        string id = Request["id"];
        string sql = "DELETE FROM leixing WHERE id='"+id+"'";
        var dmap = Db.name("leixing").find(id);
                Dao.execute(sql);
                showSuccess("删除成功");
    }
