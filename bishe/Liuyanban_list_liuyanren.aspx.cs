 ...
 *
 */
public partial class Liuyanban_list_liuyanren : AppContent
{
    // sql 查询条件
    ...
    public object getList()
    {
        ...
        sort    =  Req.get("sort" , "desc");  // 获取升序还是降序
                            ...
                            where += " AND liuyanren='"+Session["username"]+"' ";
        
        
        var     query = Db.name("liuyanban").@where(where).order(orderby,sort);
        lists    = query.page(12);
    ...
    // GET liuyanban_list.aspx?a=delete
    // 删除数据
    public void delete()
    {
        string id = Request["id"];
        string sql = "DELETE FROM liuyanban WHERE id='"+id+"'";
        var dmap = Db.name("liuyanban").find(id);
                Dao.execute(sql);
                showSuccess("删除成功");
    }
