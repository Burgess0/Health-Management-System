 ...
 *
 */
public partial class Gouwuche_list_goumairen : AppContent
{
    // sql 查询条件
    ...
    protected string sort = "";
        protected Collect<Hashtable> lists = null;  // 分页列表
            protected Hashtable total;  // 获取的统计信息
    ...
    public string getWhere()
    {
        string where = " 1=1 ";
                ...
                if(Req.getInt("shangpinid")>0){
            where += " AND shangpinid='"+Req.getInt("shangpinid")+"' ";
        }
    ...
    public object getList()
    {
        ...
        sort    =  Req.get("sort" , "desc");  // 获取升序还是降序
                            ...
                            where += " AND goumairen='"+Session["username"]+"' ";
        
        
        var     query = Db.name("gouwuche").@where(where).order(orderby,sort);
        lists    = query.page(12);


                total = Db.name("gouwuche").@where(where).field("(sum(xiaoji)) sum_xiaoji").find();
        
        
        return fastJSON.JSON.ToJSON(lists);
    }

    // GET gouwuche_list.aspx?a=delete
    // 删除数据
    public void delete()
    {
        string id = Request["id"];
        string sql = "DELETE FROM gouwuche WHERE id='"+id+"'";
        var dmap = Db.name("gouwuche").find(id);
                Dao.execute(sql);
                showSuccess("删除成功");
    }
