...
/**
 *  友情链接 模块更新页码、执行更新数据库 */
public partial class Youqinglianjie_updt : AppContent
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if(Request["f"] != null){  // 判断是否有
            handler();  // 处理友情链接模块的数据插入
            return;
        }
        var id = Req.get("id"); // 获取id
        mmm = Db.name("youqinglianjie").find(id); // 根据id从模块中获取的一行数据
    }
    ...
    protected void handler()
    {
        var post = getRequestForm();
                ...
                Db.name("youqinglianjie").update(post);
        var charuid = post["id"];
