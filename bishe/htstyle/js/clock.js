(function () {
    ...
    function solarDay() {
        ...
        var lFtv = ["0101*春节", "0115 元宵节", "0505 端午节", "0707 七夕", "0715 中元节", "0815 中秋节", "0909 重阳节", "1208 腊八节", "1224 小年", "0100*除夕"];
        var sFtv = [
            "0101*元旦",
            "0214 情人节",
            "0308 妇女节",
        ...
        for (var i = 0; i < lFtv.length; i++)
            if (lFtv[i] && lFtv[i].match && lFtv[i].match(/^(\d{2})(.{2})([\s\*])(.+)$/)) {
        ...
        if (lunarFestival == "") {
            for (var i in sFtv)
                if (sFtv[i] && sFtv[i].match && sFtv[i].match(/^(\d{2})(\d{2})([\s\*])(.+)$/)) {
        ...
        if (sFtv == "") sTermInfo = cyclical(lDObj.year - 1900 + 36) + "年 " + cDay(lDObj.month, lDObj.day);
    ...
    function showcal(id) {
        ...
        var str = "" + YYMMDD() + CurentTime() + " " + solarDay() + " " + weekday() + "";
